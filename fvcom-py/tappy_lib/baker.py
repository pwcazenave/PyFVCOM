...
class Baker(object):
    ...
    def return_cmd_doc(self, cmd):
        # Return the command name descripton for this command
        paras = process_docstring(cmd.docstring)
        ret = []
        if paras:
    ...
    def return_argnames_doc(self, cmd):
        ...
        if posargs:
            ...
            rindent = max(len(argname) + 5 for argname in posargs)
    ...
    def return_keyword_doc(self, cmd):
        ...
        if cmd.keywords:
            ...
            if any((cmd.keywords.get(a) is None) for a in cmd.argnames):
                ret.append("(specifying a single hyphen (-) in the argument list means all")
